...
const BasicDetailsseeker1 = () => {
	...
	const [dateOfBirth, setDateOfBirth] = useState(userInfo.date || "");
	const [contactNumber, setContactNumber] = useState(userInfo.contact || "");
	const [city, setCity] = useState(userInfo.city || "");
	const [email, setEmail] = useState(userInfo.email || "");
	const [description, setDescription] = useState(userInfo.disc || "");
	...
	const handleSave = useCallback(() => {
		setUserInfo({
			...
			firstName: firstName,
			lastName: lastName,
			date: formatDate(dateOfBirth), // Format date here
